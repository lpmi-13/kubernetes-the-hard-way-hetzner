...
> it would be nice if hetzner had some sort of metadata service like AWS, but this piped command should work

```
INTERNAL_IP=$(ip a | grep "10.240" | awk -F ' ' '{print $2}' | cut -d/ -f1)
```
